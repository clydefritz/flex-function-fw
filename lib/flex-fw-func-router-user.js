...
function loadIpFilterConfig(routePath) {
  const ipFilterFileName = routePath.replace('/', '').replace(/\//g, '-') + '.ips.json';
  console.log(`>> Looking for ips file named: ${ipFilterFileName}`);
  //const ipConfigFile = path.join(ipsDir, `${routePath}.ips.json`);
  const ipConfigFile = path.join(ipsDir, `${ipFilterFileName}`);
