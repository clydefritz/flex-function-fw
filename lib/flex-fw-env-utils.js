...
async function flexFunctionConfigLoader() {
    try {
        ...
        const configPath = path.resolve(rootDirectory, 'cfg/flexFunctionConfig.json');
...
async function flexEnvInitializeVars(funcName) {
    ...
    let flexFunctionConfig = '';
    console.log(`Log Point 1: ${flexFunctionConfig}`);

    try {
        flexFunctionConfig = await flexFunctionConfigLoader();
        console.log('Log Point 2');
        console.log(flexFunctionConfig); // Use the loaded configuration

        // Generate timestamp and UUID
        const start_sub_seconds = await moment().format('YYYYMMDDHHmmssSSS');
        const logUuid = uuidv4();

        // Set the flexFunctionLog.prefix in the JSON
        ...
        console.log('AFTER RUNTIME ADDITIONS');
        console.log(flexFunctionConfig);
