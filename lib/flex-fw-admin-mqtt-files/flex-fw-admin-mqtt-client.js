...
class MQTTClient {
  ...
  connect() {
    return new Promise((resolve, reject) => {
      ...
      this.client.on('connect', () => {
        global.flexlog.mqtt.log('Connected to MQTT broker');
      ...
      this.client.on('error', (error) => {
        global.flexlog.mqtt.error('MQTT Client Error:', error);
      ...
      this.client.on('message', (topic, message) => {
        ...
        if (callback) {
          ...
          global.flexlog.mqtt.warn(`No callback found for topic: ${topic}`);
        }
      });
    });
  }
  ...
  subscribe(topic, callback) {
    return new Promise((resolve, reject) => {
      this.client.subscribe(topic, (err, granted) => {
        if (err) {
          global.flexlog.mqtt.error(`Failed to subscribe to topic: ${topic}`, err);
          reject(err);
        } else if (granted && granted[0] && granted[0].qos !== 128) {
          this.subscriptions.set(topic, callback);
          global.flexlog.mqtt.log(`Subscribed to topic: ${topic} with QoS: ${granted[0].qos}`);
          resolve(granted);
        } else {
          const error = new Error(`Subscription to topic: ${topic} was rejected by the broker`);
          global.flexlog.mqtt.error(error);
          reject(error);
        }
      });
    });
    ...
    return new Promise((resolve, reject) => {
      this.client.unsubscribe(topic, (err) => {
        if (!err) {
          ...
          global.flexlog.mqtt.log(`Unsubscribed from topic: ${topic}`);
          resolve();
        } else {
          global.flexlog.mqtt.error(`Failed to unsubscribe from topic: ${topic}`, err);
          reject(err);
        }
      });
    });
  ...
  resubscribeAll() {
    ...
    this.client.end();
    global.flexlog.mqtt.log('MQTT client connection closed');
  }
}
