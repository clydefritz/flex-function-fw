...
console.log(`thisFlexFunctionIsStopping: ${global.thisFlexFunctionIsStopping}`);
...
async function flexFunctionStart() {
    ...
    console.log('Checking MQTT config');
    if (await fw_env.flexEnvGetVariable('flexFunctionMqttSupport') === true) {
        console.log(`${global.thisFlexFunctionName} supports MQTT`);
        ...
        console.log(`mqttTopic: ${mqttAdminTopic}`); 
        ...
        await mqttClient.subscribe(mqttAdminTopic, handleMqttMessageReceived, (err) => {
            if (err) {
                console.error(`Failed to subscribe to topic '${mqttAdminTopic}': ${err.message}`);
            } else {
                console.log(`Successfully subscribed to topic '${mqttAdminTopic}'`);
            }
        });

        await mqttClient.subscribe(mqttAllTopic, handleMqttMessageReceived, (err) => {
            if (err) {
                console.error(`Failed to subscribe to topic '${mqttAllTopic}': ${err.message}`);
            } else {
                console.log(`Successfully subscribed to topic '${mqttAllTopic}'`);
            }
        });
